...
* **Test 14 (`Commo_DirectionFlipRace`):** Simulates the host abruptly asserting an unexpected direction transition via `IF_DIR` (GPIO 17) mid-packet transmission. Asserts that the COMMO bridge safely tri-states its pins within the safe hardware margin to prevent physical bus contention.
